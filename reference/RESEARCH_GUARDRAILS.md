...
## AI. Внешняя модель покупается раньше, чем критерий пробуют скомпилировать
- **Temptation:** появилось словесное чтение сцены — и первым вопросом становится
  «сможет ли внешняя модель (LLM, vision, «умный» классификатор) это прочитать?».
- **Why it looked reasonable:** чтение выглядит композиционным, а не пороговым;
  модель дёшева, интерфейс типизирован, и кажется, что формализация — долгая часть.
- **What was actually wrong:** пропущен порядок проверки представления. Текстовый
  критерий сам по себе не доказывает, что он кодируем, но **автоматически
  становится кандидатом на код**. Пока попытка компиляции не сделана, неизвестно,
  существует ли вообще остаток, ради которого нужен отдельный прибор.
- **G3 case:** 096. Критерий вида «нарушение восстановлено / не восстановлено» был
  объявлен текстом, после чего построены корпус, рендерер, разметка 60 сцен и
  freeze прибора — и только потом написано правило. Детерминированное правило
  **без свободных параметров** дало κ 0,830; объявленный construct-gate κ ≥ 0,45
  взяла даже одна булева проверка одной свечи (κ 0,641) при baseline по длине
  сцены 0,262. Внешняя модель на реальных сценах так и не понадобилась.
- **Correction:** порядок работы с заявленным чтением —
  **1) literal compilation** (перевести чтение в минимальное правило без свободных
  параметров); **2) falsification of reduction** (проверить, не воспроизводит ли
  правило само чтение достаточно хорошо); **3) representation work** (только если
  редукция провалилась — выяснять, чего именно код не сохраняет);
  **4) внешняя модель** — только при содержательной причине считать, что она
  извлечёт остаток. Рыночные исходы закрыты на всех четырёх шагах.
- **Invariant:** **burden of proof перевёрнут** — не код оправдывает, почему он
  заменяет модель, а модель оправдывает, почему простого кода недостаточно.
  Правило относится к любому внешнему «умному» прибору, не к конкретному вендору.
- **Tripwire:** «сможет ли модель прочитать это чтение» — до того, как записано и
  проверено правило без свободных параметров.
...
- «сможет ли модель прочитать это чтение» (guardrail AI — сначала literal compilation:
  правило без свободных параметров, потом уже прибор)
