...
- **Первый взгляд вне корпуса — отрицательный.** Замороженное правило прогнано
  на минутах TradingView (`CME_MINI:NQ1!`) за 2026-08-24 … 2026-09-07, которых
  в корпусе нет: 11 сделок, −$10 375 в ведении v1, −$14 040 без ведения, одна
  прибыльная. Это не опровержение: такая же полоса из 10 сделок встречается в
  9,2% окон самой истории 2020–2026. Но и не подтверждение. Метод, оговорки и
  сделки — [oos_tradingview_v1.json](S-07/oos_tradingview_v1.json). Там же
  измерено, что четырёхмесячное окно вообще не может подтвердить эффект:
  при внутрикорпусном распределении лишь 75% таких окон положительны в деньгах.
...
по тем же данным ничего не подтвердит. Полезны три вещи, каждая меняет
конкретный выбор:

1. Общий расчёт позиций и риска по нескольким кандидатам сразу — совпадают ли
   убыточные дни S-07 с прежними линиями. Решает, можно ли держать их вместе.
2. Микроконтракт: тот же расчёт при цене пункта $2. Решает, доступен ли
   кандидат на счёте меньше $190 000.
3. Проверка на данных, которых правило не видело. Первый такой прогон сделан и
   отрицателен на 11 сделках (см. выше), но короткое окно ничего не решает:
   чтобы знак стал доказательным, нужно около 500 сессий — примерно два года
   новой ленты или полная история от другого поставщика. Просить у трейдера
   имеет смысл именно объём, а не ещё один месяц.
