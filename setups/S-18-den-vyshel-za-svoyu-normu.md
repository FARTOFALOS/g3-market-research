...
> **Статус для входа (2026-09-23): ЗАМЕТКА ДЛЯ БИБЛИОТЕКИ.** Единица S-18 — доллары на сессию, у S-07 и S-17 — на сделку: числа напрямую не сравниваются. Связь с S-07 слабая (r 0,18, раздел ниже); [общий расчёт](library/JOINT_S07_S17_S18.md) 2026-09-23: одним контрактом S-07, затем S-18 — +$245 на сессию, просадка $29 695; S-18 теряет $65 405 там, где утро занято позицией S-07.
...
## Состояние «остались за нормой»: возможность сама по себе?

Отдельный вопрос человека (2026-09-21); v0 не менялась, версий не заводилось. Объявление, код и вывод —
[state_s.py](S-18/state_s.py), [state_s2.py](S-18/state_s2.py), `state_s_out.txt`, `state_s2_out.txt`. Проверялась на
разрушение моя гипотеза: S — состояние, которое платит само, и свежий вход по текущей цене оправдан, сколько бы оно
уже ни длилось. Самым опасным контрпримером заранее назван один: экономика исчезает, когда путь считается от новой
цены входа, а сессия вносит один вход.

| Свежий наблюдатель, выход как у v0, после $15 | NQ 2006–2012 | NQ 2013–2019 | NQ 2020–2026 | ES 2020–2026 |
|---|---:|---:|---:|---:|
| R1 — вход на первом свежем выходе (эталон), $/сессию (t) | +1,1 (0,2) | +27,4 (2,5) | +90,0 (2,4) | +20,4 (1,0) |
| A1 — вход на первой отметке S | −1,5 (−0,4) | +21,8 (2,2) | +73,9 (2,4) | +9,1 (0,5) |
| A3 — смотрит в одну случайную из 12 отметок, входит, если видит S | −0,4 (−0,2) | +7,8 (1,8) | +14,1 (1,0) | −3,4 (−0,5) |
| вход при возрасте S 1 / 2 / 3 / 4+ отметок, $ на вход | −2 / −5 / +2 / −2 | +38 / +27 / +51 / +34 | **+232 / +193 / +108 / −46** | +40 / +1 / +32 / −58 |

- **Для наблюдателя, пришедшего в произвольное время, возможность не установлена.** A3 неотличима от нуля (t 1,0;
  без лучших 5 % дней −$97 984). Вход при возрасте 4+ (состояние длится два часа и дольше) в 2020–2026 отрицателен на
  NQ и ES, а в 2013–2019 и на ES 2006–2012 положителен: знак позднего входа между эпохами не переносится.
- **Оплачен только вход у начала состояния** — первая-вторая отметка после выхода за норму, NQ в двух последних
  эпохах. Это первый вход v0, отложенный на полчаса: средняя на 18–20 % ниже эталона при том же t, входов меньше
  (45 % сессий против 59 %), пропущены 223 сессии с немедленным возвратом (у эталона они стоили −$179 035).
- **Первоначальный вход — часть цены доступа к лучшим дням.** Из десяти лучших дней эталона 2020–2026 ($83 180)
  отложенный вход сохраняет 72 % на NQ и 77 % на ES, хотя входит во все десять. Отложить вход без потерь нельзя.
- **Что сохраняется в состоянии — не различено.** Положительный средний ход следующего получаса на отметках S
  держится во всех шести клетках (три эпохи × NQ, ES), но ни положение цены за границей (в нормах этого часа), ни знак
  продвижения за последний интервал, ни возраст состояния не упорядочивают его одинаково в разных эпохах. Доля
  возвратов падает с возрастом (0,21 → 0,06), вместе с ней падает и ход оставшихся (10,7 → 3,0 б.п.); потеря при
  возврате одна и та же, −23…−31 б.п. Деньги S-интервалов 2020–2026 ($201 305 на 700 сессиях) наполовину несут 13
  сессий, 96 % — лучшие 5 %. Отличить «состояние платит» от «экспозиция в редких длинных днях» этими данными нельзя.
- **Моя ошибка постановки.** Объявленная мной оценка A2 (случайная S-отметка внутри сессии, сессии равновесны)
  отрицательна везде (NQ −$74,8, t −2,8), но нереализуема: вероятность выбрать отметку зависит от того, сколько
  S-отметок в сессии ещё будет, то есть от будущего; она перевзвешивает сессии, где состояние вот-вот кончится.
  Сохранена как контрпример к собственному прибору, в вывод не входит. A3 и таблица по возрасту добавлены после неё.

**Какое решение это даёт.** По объявленной заранее развилке — «знание о ведении, не новый момент входа»: пока цена
остаётся за нормой, продолжать удержание оправдано; самостоятельного основания входить позже не найдено; v0 остаётся
без изменения. Развилка человека разрешается в сторону второго прочтения: «остались за нормой» — название той части
траектории, которую удерживает v0, а не отдельный адрес возможности. Это ограничено NQ и ES, получасовыми отметками,
выходом v0 и расходом $15; другой шаг отметок или другой выход не проверялись.
...
- Соседи правила — только диагностика, победитель не выбирался: окно 7 / 14 / 28 — +$156 / +$142 / +$141 на сессию;
  ...
  версией не стало.
- Повтор из каталога `setups/S-18`: `python -B run.py`, затем `checks.py`, `elsewhere.py`, `anatomy.py`, `state_s.py`, `state_s2.py`, `scenes.py`; наблюдение — `forward.py validate | overlap | ledger`; первая гипотеза —
...
Журнал начат 2026-09-21: 47 сессий 2026-05-04 … 07-10, все наблюдены, сделка в 72 % сессий, накопленный итог после
расходов +$13 780. Это не точка рассмотрения; вердикта к числу нет, ближайшая точка — 125 сессий.
