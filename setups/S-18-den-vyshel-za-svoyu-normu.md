...
1. **Распознавание необычности — необходимая часть.** Простой знак хода дня внутри нормы не несёт ничего до закрытия
   ни в одной эпохе на NQ и ES (согласуется с 063). Сторону даёт не направление дня, а выход за типичное для этого часа.
2. **Платит не вход, а то, что остаёшься.** Первый получас после свежего выхода на NQ около нуля во всех эпохах;
   ...
   в 2020–2026 доля после F выше (51 %), в двух ранних эпохах — как на NQ.
...
4. **Выход по возврату в норму — защита, а не источник денег.** После возврата прежняя сторона до закрытия меняет знак
...
m ≈ 0,08 на сессию до t ≈ 2 нужно около 625 сессий (два с половиной года). Предложение порядка — в последнем разделе.
...
**Предложение порядка последующей проверки (не принято, решение за человеком).** Заморозить v0 как есть. Территория —
сессии после 2026-05-04, накопительно, начиная с уже лежащих 47 сессий продолжения ленты (они этим правилом не
открывались). Точки рассмотрения назвать заранее: 125, 250 и 500 новых сессий; между ними не смотреть. Исторический
контекст тех же длин на NQ 2020–2026 (контекст, не порог истины): окна по 125 сессий — минимум −$26 020, 5-й
процентиль −$14 470, ниже нуля 9 % окон; окна по 250 — минимум +$5 675; окна по 500 — минимум +$41 255. Отсюда
возможное правило: на 250 новых сессиях накопленный итог ниже нуля — версия закрывается; на 500 ниже +$41 255 —
закрывается; иначе остаётся без изменений. Короткий блок не становится поводом менять правило или выбирать соседа.
S-07 v1r1 вести тем же календарём рядом, роли не смешивать.
